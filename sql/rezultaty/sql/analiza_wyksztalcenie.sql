--Analiza wg wyksztalcenia (creator: Paulina)

-- ogolne zestawienie danych badanych, gdzie
    ...
    group by pr.state, pr.county, pr.fips, cf.pst045214, cf.age295214, cf.age775214
     ...
     education as (
         select demography.state,
                demography.county,
                sum(demography.productive_age + demography.retire_age)   as voters,
                ((demography.productive_age + demography.retire_age) * (cf2.EDU685213 / 100)) /
                (sum(demography.productive_age + demography.retire_age)) as super_educated,
                ((demography.productive_age + demography.retire_age) * ((cf2.EDU635213 - cf2.EDU685213) / 100)) /
                (sum(demography.productive_age + demography.retire_age)) as middle_educated,
                ((demography.productive_age + demography.retire_age) *
                 (1 - (cf2.EDU685213 / 100 + (cf2.EDU635213 - cf2.EDU685213) / 100))) /
                (sum(demography.productive_age + demography.retire_age)) as primary_non_educated
         from demography
                  join county_facts cf2 on demography.fips = cf2.fips
         group by demography.state, demography.county, cf2.edu685213, cf2.edu635213, demography.productive_age,
                  demography.retire_age
