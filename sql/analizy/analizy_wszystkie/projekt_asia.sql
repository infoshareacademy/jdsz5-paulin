...
select area_name, max(votes) as il_gl_na_domokratow
from primary_results pr
         join county_facts cf on cf.fips = pr.fips
...
where RHI125214 is not null
  and RHI225214 is not null
  and RHI325214 is not null
  and RHI425214 is not null
  and RHI525214 is not null
  and RHI625214 is not null
  and RHI725214 is not null
  and RHI825214 is not null
group by 1
order by ilosc_glosow

-- mediana i kwartyle

select percentile_disc(0.5) within group (order by votes)  as mediana,
...
with primary_results as
    (select candidate,
            ...
            (
                case
                    ...
                    when votes between 68 and 358 then '�rednio-niski'
   ...
   , (
    ...
    when votes
   < 377 then 'niski'
    when votes between 377 and 921 then '�rednio-niski'
...
-- �redni udzia� % w wyborach w poszczegolnych stanach z podzialem na narodowosc
select distinct state,
                party,
...
-- najczesciej wystepujacy kandydaci w obu partiach -- DOKO�CZY� Z INNYMI STANAMI!!!!!!!!!!
select party, mode() within group (order by candidate)
from primary_results pr
...
select count(RHI125214) as ilo_gl_demo
from county_facts cf
...
-- korelacje liczby g�os�w i narodowo�ci
select corr(votes, RHI125214) as gl_vs_bialy,
...
-- korelacja liczby g�os�w na trumpa i narodowosci
select corr(votes, RHI125214) as gl_vs_bialy,
       corr(votes, RHI225214) as glo_vs_czarny,
       corr(votes, RHI325214) as glo_vs_indianin,
       corr(votes, RHI425214) as glo_vs_azjata,
