...
with df1 as (
    select pr.state,
           ...
           pr.votes,
           pr.party,
           cf.SBO315207 / 100 as udzial_czarnych_firm,
    ...
    group by pr.state, pr.county, pr.votes, pr.party, cf.SBO001207, cf.SBO315207),
     df2 as
         (
             select df1.county,
                    ...
                    case
                        when sum(case when df1.party like 'Repub%' then df1.votes end) >
                             sum(case when df1.party like 'Democ%' then df1.votes end) then 1
     ...
     wskazniki as (
         select unnest(
                                ...
                                within group (order by df1.udzial_czarnych_firm)) as
