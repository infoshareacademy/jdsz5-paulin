-- Analiza firm prowadzonych przez czarnoskórych (creator: Paulina)
...
--pogrupowanie hrabstw wzg.zmian zatrudnienia 
     zmiany as (select case
                       ...
                       case
                           when sum(case when w.party like 'Repub%' then w.votes end) >
                                sum(case when w.party like 'Democ%' then w.votes end) then 1
                ...
                group by zmiana_zatrudnienia_nierol_rok, w.county
                order by zmiana_zatrudnienia_nierol_rok desc
     ),
     suma as (
         select z.zmiany_zatrudnienia_nierol,
                count(z.hrabstwa) as ilo_hrabstw,
                sum(republikanie) as republikanie,
                sum(demokraci)    as demokraci
         from zmiany as z
     ...
     k2 as
         (select *,
                 s.republikanie / sum(s.republikanie) over ()::numeric d_republikanie,
...
with df1 as
     ...
     wskaz as (
         select unnest(
                                ...
                                within group (order by df1.udzial_bezrobocia)) as
                                                                              kwantyl_bezrobocie,
