--- wyjsciowe (p)
select pr.state, (select sum(pr1.votes) from primary_results pr1 where pr1.state = pr.state and pr1.party = 'Republican') as repu,
(select sum(pr2.votes) from primary_results pr2 where pr2.state = pr.state and pr2.party = 'Democrat') as demo
from primary_results pr
group by pr.state

-- wybór patrii w danym stanie (P)
select pr.state,
case
...
-- suma osób g³osuj¹cych z podzia³em na stan i wiek
select distinct pr1.state , sum(cf1.POP010210 * cf1.AGE135214) as demo_under_5, 
sum(cf1.POP010210 *cf1.AGE295214) as demo_under_18, sum(cf1.POP010210 * cf1.AGE775214) as demo_over_65,
sum(cf1.POP010210 *cf2.AGE135214) as repu_under_5, sum(cf1.POP010210 * cf2.AGE295214) as repu_under_18, 
...
-- najwiêksze argumenty dot. Narodowoœci
select distinct  state, 
greatest(
